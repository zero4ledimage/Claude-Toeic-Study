  ...
  render() {
    ...
    if (entries.length === 0) {
      ...
      entries.forEach((entry) => {
        ...
        row.innerHTML = `
          <p class="review-meta">${entry.date} · ${this._skillLabel(entry.skill_type)} · ${this._categoryLabel(entry.error_category)}</p>
          <p class="review-q">${entry.original_content}</p>
          ${entry.correction ? `<p class="review-correct">${entry.correction}</p>` : ""}
